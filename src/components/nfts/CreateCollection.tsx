...
export default function CreateCollection() {
  ...
  const addItem = async (e: any) => {
    ...
    try {
      if (!itemImage && !itemImageUrl)
        return toastError('Please upload an image')
      if (!itemSnippet) return toastError('please add a snippet')
      if (!itemSong) return toastError('please add a song')
      if (!itemDescription || !itemName || !itemPrice || !itemQuantity)
        return toastError('plesase fill the required fields')
      ...
      if (itemImgHash && itemSnippetHash && itemSongHash && itemHash) {
        console.log(`ALLHASHED!`)
      }
  ...
  const createCollection = async (e: any) => {
    ...
    try {
      ...
      const contractAdd = data.events.find(
        (e: { event: string }) => e.event === 'ERC1155Created',
      ).args[1]
      console.log(
        data.events.find((e: { event: string }) => e.event === 'ERC1155Created')
          .args[1],
      )
